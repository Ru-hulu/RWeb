...
// 分发处理函数
//定时器的作用：
//1、如果是新的连接，则挂上定时器，当定时器到时间后，request会被释放，析沟函数中这个连接可能会被删除
//2、如果是现有连接发生了事件，那么在处理事件前定时器会分离，在处理事件过程中会根据业务逻辑决定当前连接是否有必要保留。之前的定时器已经作废，迟早会被删除。
...
void handle_expired_event()
{
    ...
    while (!myTimerQueue.empty())
    {
        ...
        {
            myTimerQueue.pop();
            delete ptimer_now;//删除当前指针的时候实际上也删除的对应requestdata,关闭了文件描述符
        }
